...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AghayeCoder/tx-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/tx-ui https://raw.githubusercontent.com/AghayeCoder/tx-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/AghayeCoder/tx-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    systemctl disable tx-ui
    rm /etc/systemd/system/tx-ui.service -f
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/AghayeCoder/tx-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    /usr/local/tx-ui/tx-ui setting -username ${config_account} -password ${config_password} >/dev/null 2>&1
    /usr/local/tx-ui/tx-ui setting -remove_secret >/dev/null 2>&1
...
reset_webbasepath() {
    ...
    /usr/local/tx-ui/tx-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/tx-ui/tx-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/tx-ui/tx-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/tx-ui/tx-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/tx-ui/tx-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            LOGI "tx-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "tx-ui and xray stopped successfully"
        else
...
restart() {
    systemctl restart tx-ui
    sleep 2
    check_status
    if [[ $? == 0 ]]; then
        LOGI "tx-ui and xray Restarted successfully"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "tx-ui Set to boot automatically on startup successfully"
    else
        LOGE "tx-ui Failed to set Autostart"
    fi
...
disable() {
    systemctl disable tx-ui
    if [[ $? == 0 ]]; then
        LOGI "tx-ui Autostart Cancelled successfully"
    else
        LOGE "tx-ui Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u tx-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
        before_show_menu
        fi
        ;;
...
update_shell() {
    wget -O /usr/bin/tx-ui -N --no-check-certificate https://github.com/AghayeCoder/tx-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/tx-ui.service ]]; then
        return 2
    fi
    temp=$(systemctl status tx-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled tx-ui)
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/tx-ui/tx-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/tx-ui/tx-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/tx-ui/tx-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/tx-ui/tx-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/tx-ui/tx-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/tx-ui/tx-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/tx-ui/tx-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
show_usage() {
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}tx-ui control menu usages (subcommands):${plain}              │
│                                                       │
│  ${blue}tx-ui${plain}              - Admin Management Script          │
│  ${blue}tx-ui start${plain}        - Start                            │
│  ${blue}tx-ui stop${plain}         - Stop                             │
│  ${blue}tx-ui restart${plain}      - Restart                          │
│  ${blue}tx-ui status${plain}       - Current Status                   │
│  ${blue}tx-ui settings${plain}     - Current Settings                 │
│  ${blue}tx-ui enable${plain}       - Enable Autostart on OS Startup   │
│  ${blue}tx-ui disable${plain}      - Disable Autostart on OS Startup  │
│  ${blue}tx-ui log${plain}          - Check logs                       │
│  ${blue}tx-ui banlog${plain}       - Check Fail2ban ban logs          │
│  ${blue}tx-ui update${plain}       - Update                           │
│  ${blue}tx-ui legacy${plain}       - legacy version                   │
│  ${blue}tx-ui install${plain}      - Install                          │
│  ${blue}tx-ui uninstall${plain}    - Uninstall                        │
└───────────────────────────────────────────────────────┘"
}
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}TX-UI Panel Management Script${plain}                │
